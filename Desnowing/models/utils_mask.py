...
def input_mask(image, prob_=0.75, value=0.1):
    ...
    mask = np.random.choice([0, 1], size=(batch_size, 1, height, width), p=[prob_, 1 - prob_])
    mask = np.repeat(mask, channels, axis=1)
    noise_image = np.multiply(image, mask)
    # noise_image = noise_image - value + value * mask
    return noise_image

# def input_mask_with_noise(img_batch, sf=1, lq_patchsize=64, noise_level=15, if_mask=True, mask1=75, mask2=75):
#     # Get batch size, channels, height, and width from the input tensor
#     batch_size, channels, h1, w1 = img_batch.shape
#     img_batch = img_batch.clone()[:, :, :w1 - w1 % sf, :h1 - h1 % sf]  # mod crop
...
#
#     if noise_level > 0:
#         # Assuming add_Gaussian_noise_color works with PyTorch tensors
#         img_batch = add_Gaussian_noise_color(img_batch, noise_level1=noise_level, noise_level2=noise_level)
#     img_batch, hq = random_crop(img_batch, hq, sf, lq_patchsize)
#
#     if if_mask:
#         prob = torch.randint(mask1, mask2 + 1, (batch_size,)).float() / 100
#         # prob = torch.tensor(0.75)  # if you want to use a fixed probability
#         img_batch = input_mask(img_batch, prob_=prob)
#
#     return img_batch, hq
...
def input_mask_with_noise(img, sf=1, lq_patchsize=64, noise_level=15, if_mask=True, mask1=75, mask2=75):
    ...
    # h1, w1 = img.shape[:2]
    # batch_size, channels, h1, w1 = img.shape
    # # img = img.copy()[:w1 - w1 % sf, :h1 - h1 % sf, ...]  # mod crop
    # # h, w = img.shape[:2]
    # img = img[:, :, :h1 - h1 % sf, :w1 - w1 % sf]  # mod crop
    # _, _, h, w = img.shape
    #
    # if h < lq_patchsize*sf or w < lq_patchsize*sf:
    #     raise ValueError(f'img size ({h1}X{w1}) is too small!')
