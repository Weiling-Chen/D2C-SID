...
#        self._check_image(self.image_list)
    ...
    def __getitem__(self, idx):
        image = Image.open(os.path.join(self.image_dir, 'Snow', self.image_list[idx])).convert('L')
        # label = Image.open(os.path.join(self.image_dir, 'Gt', self.image_list[idx].split('.')[0]+'.jpg'))#srrs+jpg
        label = Image.open(os.path.join(self.image_dir, 'Gt', self.image_list[idx])).convert('L')
